...
| 任務名稱 | 完成日期 | 輸入參數 | 輸出參數 |
|---|---|---|---|
| p6b_inject_lowmass_v2（A1） | 2026-08-29 | 合成星數 N = 40,000 顆；p 真值 0.9、1.3、1.7；每點 3 次試驗；精修 3,3 | 識別度斜率 0.796；bootstrap 95% 範圍 0.593–1.019 |

p6b_inject_lowmass_v2：9 次正式注入回收全部通過完整性與邊界檢查。
三個真值的平均回收值為 0.996、1.278、1.633，偏差 RMSE 為 0.069，
低質量參數離搜尋邊界至少 0.489。結果支持低質量冪次可由資料辨識，
