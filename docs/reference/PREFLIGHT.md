...
不燒機時），跑完 `ok` 後跑 C。**2026-08-23 起兩道關卡沒過都會寫進
`QUEUE_ALERTS.md`**（不是只印在 `logs/queue_runner8.log` 等人翻），且
Gate C 不再只印警告——沒過會把狀態改記成 `gate_c_fail`（不是 `ok`），
跟 B 一樣不放進 `read_done()` 的自動重跑白名單，避免盲目重跑白燒一次
已經花掉的機時；也從原本只有 `fit_real.py` 擴充到全部五支計算腳本
（`preflight.gate_c()` 本身早就是通用實作，只是 `run_queue.py` 呼叫端
原本寫死成 `fit_real.py`）。兩道關卡都**不會**讓整條佇列停下來——沒過
就跳過這一項、機器繼續跑下一項，不留著空轉。
