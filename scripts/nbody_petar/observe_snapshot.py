...
5. **恆星型態**：`star_type` 存在且 >= 10（SSE 編號，白矮星／中子星／
   黑洞等演化終態，見 PeTar `tools/analysis/bse.py` 的
   `SSEStarParameter` 逐字核對過）的星，PARSEC 主序等時線不適用，
   v1 直接排除出光度目錄（不當成任何波段的點源）——這是已知簡化，
   理由：這些天體在真實 CMD 上通常極暗或本來就會被測光品質切掉，
   對本專案分析的 0.5-2.5 太陽質量主序段影響應該很小，但**沒有實測
   驗證過**，等真的有 125 Myr 的正式模擬快照、看到有多少顆落在這個
...
def run_self_test() -> dict:
    ...
    recall_curve = load_recall_curve(DEFAULT_RECALL_CURVE)
    fit_full, n_full = run_with(errmodel, selection_model, recall_curve)
...
def main():
    ...
    errmodel = None if args.no_noise else (
        dict(np.load(args.errmodel)) if args.errmodel.exists() else None
    )
    selection_model = None if args.no_selection else (
        selmod.load(args.selection) if args.selection.exists() else None
    )
    recall_curve = None if args.no_recall else load_recall_curve(args.recall_curve)
