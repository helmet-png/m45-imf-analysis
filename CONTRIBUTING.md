...
**規則**（原本寫在 `WORK_BOARD.md` 檔頭，2026-08-23 移來這裡跟其他
協作規範放一起，Codex 與人類協作者才讀得到）：

1. **開始任何預期要花超過一次對話（或會碰共用檔案：`pipeline/`、
   `injection_recovery.py`、`LIMITATIONS.md`、`PAPER_OUTLINE.md`、
   `queue.txt`）的工作之前**，先讀完 `WORK_BOARD.md` 的「紀錄」表，
   確認沒有人已經在做同一件事或高度重疊的事。
2. 開始工作時，在「紀錄」表**尾端加一行**，狀態填「進行中」。做完、
   暫停、或放棄時，**再加一行新的**（同任務名稱、狀態改掉），不要
   回頭改舊行——要查某任務目前狀態，從下往上找同名任務最新的一行。
...
5. **任務名稱後面要標對應的 `LIMITATIONS.md` 條目**（例如 `(A1)`），跟
   `LIMITATIONS.md` 互相參照，完整規則見本檔五之一。跟限制清單無關
   的工作（環境設定、文件整理）不用標。

**2026-08-23 起：`WORK_BOARD.md` 只放還沒做完的，做完的搬到
`WORK_BOARD_DONE.md`**——起因是「紀錄」表累積太久，想知道「現在還有
什麼要做」得先滑過大量已經結案的歷史才找得到。分工：

- **`WORK_BOARD.md`（現況板）**：只放狀態是「進行中」「暫停」「交接中」
  「疑義」的紀錄行，以及全部「待認領工作」表格（定義上都是還沒做完的）。
- **`WORK_BOARD_DONE.md`（封存）**：只放狀態已經是「完成」（或等價的
  結案狀態，例如「已解決」「確認結案」）的紀錄，純粹歷史記錄。

**搬移時機與做法**：一個任務在「紀錄」表的最新一行狀態變成「完成」時，
**同一次 commit 裡把這個任務從第一行到最後一行的完整歷史**從
`WORK_BOARD.md` 剪下、貼到 `WORK_BOARD_DONE.md` 的紀錄表尾端——保留
完整過程，不是只搬最後一行，中間的卡關、修正、CodeRabbit 抓到的錯誤
都是有價值的記錄，跟這個任務的最終結論放在一起才有意義。「待認領
工作」表格裡的項目做完時，同樣把整行搬過去，不要留著劃線＋「已完成」
的寫法佔著現況板版面。**這條不影響「不要回頭改舊行」的原則**：搬的
是狀態已經是終態、確定不會再變動的歷史，不是回頭竄改內容；搬移本身
是一次新的 append（貼到 `WORK_BOARD_DONE.md` 尾端），跟 append-only
的精神一致。
