...
**2026-08-23 起新增**：`WORK_BOARD.md` 累積到 449 行後，歷史紀錄
（誰做過什麼）跟待辦事項（誰現在在做什麼、還剩什麼）混在同一份
append-only 文件裡，越來越難一眼看出「現在到底剩什麼還沒做」——
要先讀完幾百行歷史才能確認某個任務算不算已經有人做過。拆成兩份：

- **`WORK_BOARD.md`**：只放正在執行或還沒執行的工作。認領規則、
  疑義處理、算力池歸屬這些「怎麼用這份文件」的說明也留在這裡。
- **`WORK_BOARD_DONE.md`**：只放已經達成、結案的事，包含原本的
  `## 紀錄`／`## 紀錄（續）`兩張 append-only 歷史留痕表。

**判斷一個任務算不算「已完成」該搬過去**：驗收標準已經達成、或
使用者／後續查證已經明確確認結案，都算——不要求「絕對沒有任何
後續延伸工作」，只要求「這一行描述的具體任務本身做完了」。行內
文字如果提到「若要升級／若要量化，還可以做 X」，那是**選配的延伸
工作**，不影響這一行本身歸類為已完成；但如果行內明講「仍待做」
「目前阻塞」「還沒有」這種**這個任務本身的必要部分沒做完**，就不
算，留在 `WORK_BOARD.md`。判不出來就當還沒完成，留在待辦那份——
錯誤地把還沒做完的事標成已完成，比錯誤地把做完的事留在待辦，代價
大得多（前者會讓人以為不用做了）。

**具體操作**：
1. 一個任務真正做完時，**把 `WORK_BOARD.md` 裡這個任務名稱底下所有
   的歷史狀態列都剪下**（不只是最後、狀態填「完成」的那一行——同一
   任務若經歷過「進行中→暫停→完成」這種多筆狀態更新，中間那幾筆
   舊列也要一起搬走，不能只搬最新那筆、把「進行中」「暫停」這種舊
   狀態留在 `WORK_BOARD.md` 裡製造誤判），依原本出現的先後順序，
   依序貼到 `WORK_BOARD_DONE.md`「紀錄（續）」表尾端（append-only，
   不要改內容、不要打亂順序），`WORK_BOARD.md` 這裡全部刪掉，不要
   留著（2026-08-23 CodeRabbit review：原本只講「對應那一行」會漏掉
   這種情況）。
2. 只是狀態更新（進行中→暫停、換人接手）不算完成，**兩份都不用動
   結構**——照零的規則在 `WORK_BOARD.md` 對應表格加一行新狀態即可，
   維持 `WORK_BOARD.md` 原本「同任務名稱、狀態改掉、不回頭改舊行」
   的慣例。
3. 待辦表格裡若有一整個表已經全部項目都完成（例如某個子計畫的所有
   小任務都結案了），可以把整個表搬到 `WORK_BOARD_DONE.md` 新增一個
   小節，不用逐行搬。
