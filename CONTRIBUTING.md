...
**2026-08-23 起新增，2026-08-24 訂正表格格式（見下方「表格格式」一節）**：
`WORK_BOARD.md` 累積到 449 行後，歷史紀錄（誰做過什麼）跟待辦事項
（誰現在在做什麼、還剩什麼）混在同一份 append-only 文件裡，越來越
難一眼看出「現在到底剩什麼還沒做」——要先讀完幾百行歷史才能確認
某個任務算不算已經有人做過。拆成兩份：

- **`WORK_BOARD.md`**：只放正在執行或還沒執行的工作。認領規則、
  疑義處理、算力池歸屬這些「怎麼用這份文件」的說明也留在這裡。
- **`WORK_BOARD_DONE.md`**：只放已經達成、結案的事，是唯一保留
  append-only 歷史留痕（誰在什麼時候做了什麼、中間卡過哪裡、
  CodeRabbit 抓到什麼錯）的地方。
...
「還沒有」這種**這個任務本身的必要部分沒做完**，就不算，留在
`WORK_BOARD.md`。判不出來就當還沒完成，留在待辦那份——錯誤地把
還沒做完的事標成已完成，比錯誤地把做完的事留在待辦，代價大得多
（前者會讓人以為不用做了）。

**表格格式（2026-08-24 訂正，取代原本「同任務名稱、狀態改掉、不回頭
改舊行」的做法）**：`WORK_BOARD.md` 只有**一張統一大表格**，不要再
分成好幾個各自不同欄位的小表格——同一種資訊放同一個地方，找東西
才不用先猜它在哪一張表。欄位固定五個：
...
- **狀態只有兩種**：「進行中」與「尚未進行」。已完成的不會出現在
  這份文件——完成就整行搬去 `WORK_BOARD_DONE.md`（見下方具體操作）。
- **每個任務在表格裡只有一行**，不是每次狀態變動就加一行新的。
  狀態從「尚未進行」改成「進行中」時，**直接覆蓋那一格**，開始日期
  也一併補上；不要用刪除線劃掉舊狀態再寫新的、也不要在同一格裡
  疊加好幾輪的更新說明——舊的過程資訊（卡在哪裡、試過什麼、
  CodeRabbit 抓到什麼）搬去 `WORK_BOARD_DONE.md` 對應任務的歷史
  記錄裡，`WORK_BOARD.md` 的表格本身只反映**現在的狀態**。
- **表格欄位裡不要塞解釋性文字**：任務名稱後面的括號只放對應的
  `LIMITATIONS.md` 條目代碼（例如 `A1`），不要在括號裡寫一整句話。
  輸入／輸出參數只放實際的指令旗標與檔名，不要夾帶「為什麼這樣做」
  這類說明。
- **每個任務在表格下方另外有一段獨立的說明文字**（標題用任務名稱），
  寫清楚這個任務具體要解決什麼問題、用什麼方法、目前卡在哪裡、
  預計要跑多久——耗時只在有實測依據時才寫具體數字，查不到依據就
  寫「未查證」，不要憑感覺湊一個數字（這條本來就是全域規則，這裡
  特別強調是因為表格本身沒有空間放這些內容，容易被跳過不寫）。
- **不要用粗體強調**：無論表格還是說明文字，都用平鋪直敘的句子，
  不要靠粗體標重點——重點寫進句子本身的用字就好。

**具體操作**：
1. 一個任務真正做完時，把 `WORK_BOARD.md` 裡那一行（表格裡的一行
   加上下方對應的說明段落）整段剪下，依原本出現的先後順序貼到
   `WORK_BOARD_DONE.md`「紀錄（續）」表尾端（append-only，不要改
   內容、不要打亂順序），`WORK_BOARD.md` 這裡全部刪掉，不要留著。
2. 只是狀態更新（尚未進行→進行中、換人接手、卡住了）不算完成，
   直接覆蓋 `WORK_BOARD.md` 裡那一行的表格儲存格與下方說明文字，
