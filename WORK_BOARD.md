...
3. **Kaggle 也裝不下**：這一點繫於單次擬合時數，而上面說明該數字目前
   無法佐證，所以這一條也一併降級為待確認。方向上仍成立的部分是：
   照 `p6b` 那樣按軸拆片救不了「單次擬合」本身，只能拆重複次數。
...
**之後真的要跑它之前**：先把 `results/profile_lowmass.npz`（2026-08-15
的舊檔、無 manifest、是未精修的粗網格結果）移開。現在有兩道保護會擋下來，
但兩道都只是「擋住」不是「自動處理」，還是要人去移檔案：PR #89 的
`checkpoint.check_manifest()`（無 manifest 的舊檔不再靜默沿用）與 PR #96
的 `preflight.py --gate c`（沒有 manifest 時照樣判定量化簽章）。
