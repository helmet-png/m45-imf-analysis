...
|---|---|---|
| `rv_outlier_member_exclude`（A6，優先度高——影響頭條數字，`wd_exclude_rerun` 的剩餘部分） | 白矮星那顆（`source_id=66697547870378368`）已經用 `assign_masses()` 的顏色一致性檢查處理掉（見 A6，`yutunglan/x64-pipeline-verify` 分支之後、`claude/wd-exclude-rerun` PR #43）。**這顆不行**：`source_id=64895139073954944` RV 離群（19.5σ）但顏色跟真成員無異，顏色檢查抓不到，需要獨立於 `assign_masses()` 的機制排除（例如成員表層級的 RV 交叉核對名單），不要想用同一個顏色門檻硬兜 | 確認 `source_id=64895139073954944` 確實被排除（用什麼機制、為什麼），並量化排除前後對頭條 alpha 的影響（不管差異大小，都要有數字） |
| `bright_outlier_investigate`（D9，優先度中——已確認會影響一個目前引用的頭條數字，只是影響量小） | `source_id=68409590552589184`（G=7.386）在三個測過的質量範圍下都被 `assign_masses()` 的顏色一致性檢查標記，偏離主序色 1.3–1.4 星等。比照 A6 查白矮星的方法，先查 Gaia 官方分類機率／RUWE／`non_single_star` 旗標，查不到再考慮是不是極端雙星或前景/背景污染源。**跟 `rv_outlier_member_exclude` 是不同顆星**，不要合併處理——這顆是顏色離群，那顆是 RV 離群，篩選機制不同 | 對這顆星的身分給出一個有根據的結論（巨星／污染源／極端雙星／其他），並確認排除或保留它對 `traditional_accounting.py` 主表頭條數字（目前 +0.013 的位移）有沒有需要調整的地方 |
| `rv_binary_investigate`（C3） | 先查文獻（Mermilliod 系列、或任何 M45 的分光雙星軌道編目）有沒有現成的已知 SB 名單能直接核對這 56 顆離群星；查不到再考慮用 `f_bin=0.45` 加上 Kroupa/Sana 週期分布估計「隨機取樣到大 RV 偏移」的期望比例，看 11.2% 這個數字是不是分光雙星就能解釋掉，不用假設一定是污染 | 對這 56 顆給出一個有根據的結論（分光雙星可以解釋多少比例、剩下多少無法解釋），不是停在「不知道」 |
| `c20_reconcile_disagree_set`（C20） | 先找出當初「20 顆判定分歧」是怎麼算出來的（多半在某次 session 的過程記錄或 git log 裡，搜尋 `git log --all --grep`／舊的 `LIMITATIONS.md` 版本），跟 `comparison.csv` 現在重算出的 367 顆分歧對照，搞清楚差在哪個門檻或涵蓋範圍 | 重建出跟原始「20」一致的分歧集合，才能真的用 `gaia_radial_velocity.py` 查這批星 |
...
| `selection_color_dependence_fix`（C18） | `pipeline/selection.py` 目前 BP/RP 流量超額那一刀只用星等相依的隨機移除近似，殘留 +0.014 的顏色相依效應未建模。需要先畫出殘留量隨顏色的分布（不是只有一個平均值 0.014），再決定要不要把選擇函數從「只吃星等」擴充成「吃星等＋顏色」的二維模型 | 顏色相依的殘留量被量化成一個函數（不只是常數），且套用擴充後的選擇函數不會讓已驗證安全的整體驗收（+0.018／0.048／−0.362 vs −0.388，見 C18 條目本身)劣化 |
