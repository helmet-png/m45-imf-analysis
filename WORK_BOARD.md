...
|---|---|---|
| `wd_exclude_rerun`（A6，優先度高——影響頭條數字） | 幫 `pipeline/step5_imf.assign_masses()` 加顏色一致性檢查：算出每個 G 星等對應的主序顏色，觀測顏色偏離超過某個門檻（先抓合理值，例如 0.3–0.5 星等，這個門檻本身要留一句理由）就回傳 NaN，跟現有處理範圍外星等的機制一致。**不要用手動從 `cmd_members.csv` 刪掉那一行星頂替**——理由見 A6 條目。修好後重跑 `p2_final2` 等級的頭條擬合，比較加了顏色檢查前後 alpha 有沒有變 | 確認 `source_id=66697547870378368` 被新的檢查機制正確排除（得到 NaN，不是錯誤質量），且排除前後頭條 alpha 的差異被量化寫進 `LIMITATIONS.md`（不管差異大小，都要有數字，不能只寫「應該影響很小」） |
