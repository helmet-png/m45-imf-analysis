...
|---|---|---|
| `bright_outlier_investigate`（D9，優先度中——已確認會影響一個目前引用的頭條數字，只是影響量小） | `source_id=68409590552589184`（G=7.386）在三個測過的質量範圍下都被 `assign_masses()` 的顏色一致性檢查標記，偏離主序色 1.3–1.4 星等。比照 A6 查白矮星的方法，先查 Gaia 官方分類機率／RUWE／`non_single_star` 旗標，查不到再考慮是不是極端雙星或前景/背景污染源 | 對這顆星的身分給出一個有根據的結論（巨星／污染源／極端雙星／其他），並確認排除或保留它對 `traditional_accounting.py` 主表頭條數字（目前 +0.013 的位移）有沒有需要調整的地方 |
