...
|---|---|---|
| `wd_exclude_rerun`（A6，優先度高——影響頭條數字） | 幫 `pipeline/step5_imf.assign_masses()` 加顏色一致性檢查：算出每個 G 星等對應的主序顏色，觀測顏色偏離超過某個門檻（先抓合理值，例如 0.3–0.5 星等，這個門檻本身要留一句理由）就回傳 NaN，跟現有處理範圍外星等的機制一致。**不要用手動從 `cmd_members.csv` 刪掉那一行星頂替**——理由見 A6 條目。**這個顏色檢查只抓得到白矮星那顆**（`source_id=66697547870378368`，顏色明顯偏藍）——第二顆候選（`source_id=64895139073954944`，RV 離群但顏色跟真成員無異，見 A6）光靠顏色檢查抓不到，需要另外處理：確認的話用獨立於 `assign_masses()` 的機制排除（例如成員表層級的 RV 交叉核對名單），不要想用同一個顏色門檻硬兜。修好後重跑 `p2_final2` 等級的頭條擬合，比較排除前後 alpha 有沒有變 | (1) 確認 `source_id=66697547870378368` 被顏色檢查正確排除（得到 NaN）；(2) 確認 `source_id=64895139073954944` 是否確實排除（用什麼機制、為什麼）；(3) 排除前後頭條 alpha 的差異被量化寫進 `LIMITATIONS.md`（不管差異大小，都要有數字，不能只寫「應該影響很小」） |
