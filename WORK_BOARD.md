...
**2026-08-23 更新**：`data/m45_r5_g18_plx4.csv` 已成功抓到（6,956 顆未篩選，2026-08-21 11:49），阻擋這個任務的伺服端 COUNT(*) 逾時已用 `--top` 旗標解決。**跑過一次端到端驗證**（示範規模，`--procs 4 --n-syn 5000 --refines 3 --values 0.7`）：先用 `--n-syn 500 --refines 1` 試跑撞牆（`WallError`：alpha 貼在下界 1.500），換成文件建議的示範規模後正常跑完，`threshold=0.70 → alpha=2.633、logage=8.000、A_V=0.500、lnP=1266.4`，耗時 23,142 秒（約 6.4 小時）——**證實撞牆是極端小樣本（n_syn=500）造成的假性失敗，不是腳本問題，示範規模的整條流程（含這次解掉的資料抓取阻塞）真的能跑通**。只有單一門檻值，還不構成敏感度結論（至少要 2 個門檻才有斜率可看），**完整 5 門檻 × `--n-syn 40000 --refines 3,3` 的正式掃描還沒做**：以 23,142 秒（n_syn=5000）外推，n_syn=40000 單一門檻估計約 50+ 小時，5 個門檻合計數百小時，跟「第 2 步四項到齊後新解鎖的工作」表上方同樣的機器分配原則一樣——**不適合排本機 8 核佇列**，適合排到有較多核心的機器或再分片，待認領。

**2026-08-23 再更新：已改派到 gcp1（見下方「算力來源」新增小節）**，本機
`queue.txt` 這一項已停用避免重複算。正式規模先排 0.6／0.7 兩個門檻當
第一批（`d2_membership_threshold_p06_p07_retry`），觀察真實耗時後再
決定要不要把剩下的 0.5/0.8/0.9 也排進去，不用等本機或 Kaggle 有空。
