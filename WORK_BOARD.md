...
針對現在三個算力池（gcp1、6 個 Kaggle 帳號）的實際使用狀況給派工建議。
...
1. `limepy_radial_crosscheck` 補做正式版——B5 的探索性版本
   ...
   閒置機器（包括本機）都能做，優先度最高但成本最低，建議最先認領。
2. `nbody_prior_from_radial` 的初步校準網格——A5 解除條件的另一半
   （第 5 步 N-body 交叉驗證），前置的三個定義不一致已解決，缺的是
   真的跑一組小規模模擬網格。跟第 1 項不衝突，可以平行進行。
3. `extinction_form_test`（C5，現役缺陷．優先度高）——已經在 gcp1
   佇列裡進行中（`c5_davform_lognormal`／`c5_davform_truncexp`），
   不用重新排，等它跑完直接看結果、寫進 LIMITATIONS.md C5。
...
**第三階段（結構性補強與敏感度測試，不影響現有頭條結論，但論文需要
誠實揭露這些還沒測過）**：
