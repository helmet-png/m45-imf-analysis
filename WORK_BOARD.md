...
|---|---|---|
| `bright_outlier_investigate`（D9，優先度中——已確認會影響一個目前引用的頭條數字，只是影響量小） | **2026-08-13 本機可查的部分已查完**（`check_bright_outlier_kinematics.py`）：RUWE=1.098（乾淨）、`non_single_star=0`、BP/RP 測光超額因子在 Evans+2018 標準品質帶內（不算測光異常）、pmra／視差運動學一致（<1.6σ），**只有 pmdec 明顯異常（3.9–4.8σ，依用哪種離散度而定）**。外部目錄交叉比對（SIMBAD／Gaia DR3 non_single_star 相關表）在這台機器連不上（SSL 憑證驗證失敗），下一個能連網路的環境**直接查外部目錄**，不用重跑本機這幾項 | 對這顆星的身分給出一個有根據的結論（巨星／污染源／極端雙星／其他），並確認排除或保留它對 `traditional_accounting.py` 主表頭條數字（目前 +0.013 的位移）有沒有需要調整的地方 |
