...
| ~~`stick_out_fraction_constraint`（D13，成本評估）~~ **2026-08-20 評估已完成** | 見 `docs/planning/PLAN_D13_stickout_成本評估.md`：不是「小改動、大概率有用」，是「小驗證（幾乎零成本）、中不確定、若成立則大改動（整個生產似然函數要改、headline 全部重跑）」。建議先做零成本的診斷（量出 M45 真實資料的凸出星比例 `f_SO_obs` 統計量夠不夠），再決定要不要投入正式整合 | 已達成：成本評估文件已寫好，下一步是否啟動由認領的人依文件裡的步驟 1（診斷數字）決定 |
...
**2026-08-20「進行中」認領（本 session）**：步驟 0（本行）、1（見上方 D8
2026-08-20 更新，已滿足）、2、3 已完成，4 部分完成（Tang 已核對、
Kraus & Hillenbrand 仍卡住），5 尚未開始，交給下一個認領的 session／
自己接著做：

- **步驟 3（HR23 名稱與品質）結果**：`Hunt & Reffert (2023)` 目錄裡
  Coma Ber 的正式名稱是 **`Melotte_111`**（底線，跟 `NGC_3532` 同慣例，
  不是「Coma Ber」也不是「Mel 111」）。VizieR TAP 查詢
  `J/A+A/673/A114/clusters` 得到 HR23 自己的擬合值：logAge50=8.816
  （654.8 Myr）、AV50=0.071 mag、dist50=85.25 pc、MOD50=4.708。
  `J/A+A/673/A114/members` 表：**`Prob>=0.7`（Tier 1 預設門檻）只有
  69 顆成員**（全機率範圍共 271 顆）——遠少於 M45（1,078 顆）、NGC 3532、
  Praesepe，樣本小很多，統計誤差會明顯較大，跑 Tier 1 時要如實記錄
  這個樣本量差異，不要跟其他星團的誤差棒直接並排比較而不提這件事。
  **這台機器查證時撞到一個環境問題**：裸 `urllib.request` 對
  `tapvizier.cds.unistra.fr` 會丟 `SSL: CERTIFICATE_VERIFY_FAILED`
  （系統憑證庫沒有裝好），要傳 `ssl.create_default_context(cafile=
  certifi.where())` 才能連上，`cluster_imf_tier1.py` 的 `vizier_query()`
  目前沒有這個 context 參數——**在這台機器上真的要跑 `--refresh` 之前，
  必須先幫 `vizier_query()` 補上 certifi context，不然會直接連線失敗**
  （其他機器若系統憑證庫正常則不受影響，不確定是不是只有這台機器的問題）。
- **步驟 2（動力學年齡 τ）結果**：用 `dynamics_estimate.py --age 654.8
  --distance 85.25 --sweep` 粗算（質量/半質量半徑都還沒有 Coma Ber
  專屬的文獻值，先用保守估計）：即使保守取 M_cl=100 M☉（遠低於 M45
  的 600–1000 M☉ 假設區間，Coma Ber 是已知的稀疏、蒸發中星團），
  r_h=4 pc，算出 **t_rh=60.3 Myr，τ=age/t_rh=10.9**——遠大於 1，
  而且遠大於 M45 本身的 τ=1.11（見 A5）。M45 的 τ 才剛過 1 就已經
  判定「分層一定發生了」，Coma Ber 的 τ 是它的近 10 倍，**Tier 2
  動力學校正幾乎確定必要**，不是「大機率」而已，起跑前直接排入
  Tier 2 規劃、不用再猶豫要不要做 Tier 1-only 版本。
  **意外發現，需要下一步處理**：同一次計算算出 Coma Ber 在目前假設
  下的 Jacobi（潮汐）半徑只有 **6.48 pc = 4.35°**（85 pc 距離處），
  但 M45／NGC 3532／Praesepe 都沿用的**預設 5° 搜尋半徑**，換算成
  Coma Ber 的物理距離是 7.44 pc，**已經是 Jacobi 半徑的 1.15 倍**
  ——直接沿用 5° 抓 Gaia 資料，範圍會超出潮汐半徑，混進潮汐尾／
  背景污染的機率比 M45 高。下一步要嘛把搜尋半徑縮小到 4.35° 左右
  （對齊 Jacobi 半徑），要嘛保留 5° 但在方法論裡明確承認會收進潮汐尾
  外緣（Coma Ber 有已知的延伸潮汐尾結構，這也可能是想要的東西，
  但要是刻意決定，不是預設沒想過）——這個取捨留給實際跑 Tier 1 的人
  決定，不要跳過這一步直接照抄 M45 的 5°。
- **步驟 4（Kraus & Hillenbrand 2007）仍卡住**：需要使用者提供正確的
  那篇（*The Astronomical Journal*, 134, 2340），之前收到的 PDF 是同作者
  同年份但不同期刊/不同星團的 *ApJ* 662, 413，見上一行紀錄。在拿到正確
  PDF 前，Kraus & Hillenbrand 這邊只能引用 Tang et al. (2019) 轉述的
  二手數字（α=0.6±0.3，0.1–1.0 M☉），不能當已核對口徑的基準線。
