...
| 任務名稱 | 狀態 | 開始日期／指派時間 | 輸入參數 | 輸出參數 |
|---|---|---|---|---|
| limepy_radial_crosscheck（A5、B5） | 完成 | 指派時間：2026-08-13；完成時間：2026-08-25 | King 模型結構參數 φ0 = 3.44、r0 = 2.50 pc；觀測 α(<r) 於 r = 1°、2°、3°、全樣本（5.1°）四個累積半徑（正式五次重複版） | 模型 alpha 在四個半徑低估觀測 0.260–0.431（觀測樣本散布的 2.73–4.40 倍），方向上皆支持核心到約 2° 的上升；2° 外觀測近乎變平、模型仍緩升。細節見 LIMITATIONS.md B5、`docs/planning/LIMEPY_RADIAL_CROSSCHECK_FINAL_2026-08-25.md` |

limepy_radial_crosscheck：LIMEPY 多質量平衡模型本身已經擬合完成
（phi0=3.44、r0=2.50 pc，reduced chi²=0.75），只讀既有 LIMEPY 預測與
四份正式五次徑向結果（未重跑 IMF／前向模型）做讀檔比較，2026-08-25
完成。**明確保留的限制**：模型端尚無 bootstrap 誤差棒、兩端共用
成員、三質量箱回歸跟個別星前向估計不是同一估計器——這個低估的量級
不能直接稱為正式 sigma 顯著性，只是初步顯示差距大於目前量到的
seed-to-seed 散布，不把這個診斷寫成正式模型否決。
