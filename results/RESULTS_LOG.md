...
| 2026-08-13 | Claude session（新協作者機器，x64，Yu Tung Lan） | `scripts/diagnostics/limepy_multimass.py`（獨立 venv，見腳本開頭環境說明） | 05b3e80 | `limepy_multimass.npz` | PDMF→IMF 第 3 步：LIMEPY 多質量 King 模型初步擬合（694 顆星、3 質量區間）。phi0=3.52、r0=2.49 pc、擬合總質量 534.9 M_sun，reduced chi^2=8.90（**擬合不算好，數字方向可信、精確值待改進**）；模型潮汐半徑 r_t=20.16 pc（模型內部量，不等於 dynamics_estimate.py 算出的 Jacobi 半徑 12.96 pc），預測目前 Gaia 樣本邊緣（11.70 pc）到 r_t 之間還有 21.2 M_sun（模型總質量的 4.0%） |
