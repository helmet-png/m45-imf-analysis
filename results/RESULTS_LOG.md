...
| 2026-08-14 | Claude session（本機） | `inject_lowmass.py --only 1.3 --trials 1 --tag _p6b4_retest`（D5 補測） | f91b091 | `inject_lowmass_p6b4_retest.npz` | D5 補測：原本 8 筆裡有 1 筆（`p_true=1.3` 第 3 次試驗）logage=8.3 貼到 PARSEC 網格真實涵蓋上界（8.25）。這次單獨補測 `p_true=1.3` 一次，logage=8.133，安全落在範圍內，**沒有再貼牆**；p_recovered=1.367（偏差 +0.067）、alpha_rec=2.433（偏差 +0.083），量級跟原本 7 筆一致，支持「原本貼牆是單次隨機波動」的解讀，但只補測 1 次不是 3 次，不能完全排除偶爾仍會貼牆 |
